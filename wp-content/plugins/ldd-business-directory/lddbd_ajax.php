...
// Add the business and all the corresponding elements assigned to it into the database.
    ...
    /**
     * Fixing the logo upload
     */

    $logo = '';
    if ( 0 === $_FILES['logo']['error'] ) {

        $filetype = wp_check_filetype( $_FILES['logo']['name'] );
        if ( in_array( $filetype['type'], array( 'image/jpg', 'image/jpeg', 'image/png', 'image/gif' ) ) ) {
            ...
            $copy = $wp_upload_dir['basedir'] . '/lddbd-logos/' . $filename;
            ...
            if ( WP_Filesystem( $creds ) ) {
                ...
                if ( $wp_filesystem->copy( $_FILES['logo']['tmp_name'], $copy ) )
                    $logo = 'lddbd-logos/' . $filename;
            }
