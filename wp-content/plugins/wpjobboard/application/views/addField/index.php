<?php $this->slot("title", __("Additional Fields", WPJB_DOMAIN)); ?>
<?php $this->_include("header.php"); ?>

<div class="wpjb-buttons">
    <a href="<?php echo $this->_url->linkTo("wpjb/addField", "edit"); ?>"class="button button-highlighted">
        <?php _e("Add New Additional Field", WPJB_DOMAIN) ?>
    </a>
</div>
...
<script type="text/javascript">
    Wpjb.DeleteType = "additional field";
</script>
...
<table cellspacing="0" class="widefat post fixed">
    ...
    <<?php echo $tx; ?>>
        <tr>
            ...
            <th style="" class="" scope="col"><?php _e("Label", WPJB_DOMAIN) ?></th>
            <th style="" class="" scope="col"><?php _e("Type", WPJB_DOMAIN) ?></th>
            <th style="" class="" scope="col"><?php _e("Hint", WPJB_DOMAIN) ?></th>
            <th style="" class="" scope="col"><?php _e("Is Required", WPJB_DOMAIN) ?></th>
	...
	<tr valign="top" class="<?php if($i%2==0): ?>alternate <?php endif; ?>  author-self status-publish iedit">
            ...
            <td class="post-title column-title">
                <strong><a title='Edit "(<?php esc_html_e(" ".$item->label) ?>)"' href="<?php echo $this->_url->linkTo("wpjb/addField", "edit/id/".$item->getId()); ?>" class="row-title"><?php echo esc_html($item->label) ?></a></strong>
                <div class="row-actions">
                    <span class="edit"><a title="<?php _e("Edit", WPJB_DOMAIN) ?>" href="<?php echo $this->_url->linkTo("wpjb/addField", "edit/id/".$item->getId()); ?>"><?php _e("Edit", WPJB_DOMAIN) ?></a> | </span>
                    <span class=""><a href="#<?php echo $item->getId() ?>" title="<?php _e("Delete", WPJB_DOMAIN) ?>" class="wpjb-delete"><?php _e("Delete", WPJB_DOMAIN) ?></a> | </span>
                </div>
            </td>
            <td class="author column-author"><?php echo($item->getFieldType()) ?></td>
            <td class="author column-author"><?php echo esc_html($item->hint) ?></td>
            <td class="categories column-categories"><?php echo ($item->is_required) ? __('Yes', WPJB_DOMAIN) : __('No', WPJB_DOMAIN) ?></td>
...
<div class="tablenav">
    <div class="tablenav-pages">
        ...
        echo paginate_links( array(
                'base' => $this->_url->linkTo("wpjb/addField", "index/page/%_%"),
                'format' => '%#%',
                'prev_text' => __('&laquo;'),
                'next_text' => __('&raquo;'),
                'total' => $total,
