...
function coolwp_update_valuation($id, $data = array()) {
    ...
    if (isset($data['valuation_date'])) $data['valuation_date'] = date_i18n('Y-m-d', $data['date'], true);
    if (isset($data['oonline_date'])) $data['oonline_date'] = date_i18n('Y-m-d', $data['date'], true);
...
function coolwp_get_valuations($query = array()) {
    ...
    $where_sql = 'WHERE 1=1';
    if (!empty($id)) $where_sql.= $wpdb->prepare(' AND id=%d', $id);
    if (!empty($company_id)) {
        //Force $company_id to be an array   强制$company_id 为一个数组
        if (!is_array($company_id)) $company_id = array(
            $company_id
        );
    ...
    $since = absint($since);
    $until = absint($until);
    if (!empty($since)) $where_sql.= $wpdb->prepare(' AND valuation_date >= %s', date_i18n('Y-m-d', $since, true));
    if (!empty($until)) $where_sql.= $wpdb->prepare(' AND valuation_date <= %s', date_i18n('Y-m-d', $until, true));
    /* SQL Order */
    //Whitelist order  白名单
    ...
    foreach ($pieces as $piece) $$piece = isset($clauses[$piece]) ? $clauses[$piece] : '';
